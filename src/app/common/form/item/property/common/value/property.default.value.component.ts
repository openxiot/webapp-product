  ...
  providers: [
    {
      ...
      useExisting: PropertyDefaultValueComponent,
      multi: true
    }
  ],
})
export class PropertyDefaultValueComponent implements ControlValueAccessor {

  @Input() updatable: boolean = false;
  @Output() changed: EventEmitter<void> = new EventEmitter<void>();

  // 组件内部维护的值
  _value = signal<PropertyDefaultValue>(new PropertyDefaultValue());

  // 禁用状态
  isDisabled = false;

  // 定义变化回调和触摸回调
  onChange: (value: PropertyDefaultValue) => void = () => {};
  onTouched: () => void = () => {};
  ...
  // 获取当前值
  get value(): PropertyDefaultValue {
    return this._value();
  }

  // 设置当前值，并通知外部变化
  set value(val: PropertyDefaultValue) {
    if (val !== this._value()) {
      this._value.set(val);
  ...
  protected onMinChange(value: number) {
    ...
    this._value.update(v => Object.assign(new PropertyDefaultValue(), v, {min: value}));
  ...
  protected onMaxChange(value: number) {
    ...
    this._value.update(v => Object.assign(new PropertyDefaultValue(), v, {max: value}));
  ...
  protected onStepChange(value: number) {
    ...
    this._value.update(v => Object.assign(new PropertyDefaultValue(), v, {step: value}));
  ...
  writeValue(value: PropertyDefaultValue): void {
