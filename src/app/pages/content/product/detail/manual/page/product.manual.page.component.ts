...
import {LifeCycle, ProductManualPage} from '@openxiot/xiot-core-spec-ts';
...
export class ProductManualPageComponent implements OnInit, OnDestroy {
  ...
  @Output() changed = new EventEmitter<ProductManualPage>();
  ...
  protected customUpload = (item: NzUploadXHRArgs): Subscription => {
    ...
    const uploadFlow$ = this.service.getFileUploadUrl(this.productId, 'manual', item.file.name).pipe(
      // 切换到上传请求
